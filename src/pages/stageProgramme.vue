<template>
  ...
  <aside id="settings" v-show="showSettings">
    <p><button @click="test()">10倍速測試</button></p>
    <p>
      正文模板
      <button @click="clearData()">恢復預設正文</button>
    </p>
    <textarea cols="80" rows="20" v-model="programme.inputRaw"></textarea>

    <p>總縮放<input type="number" v-model="programme.style.scale" step="0.1" min="0.5" /></p>
    <p>每行高<input type="number" v-model="programme.style.lineHeight" step="1" min="20" /></p>
    <p>允許使用者滾動<input type="checkbox" v-model="enablescrollBar" />（該選項不會儲存）</p>
    <p>
      自定義CSS
      <button @click="clearStyle()">恢復預設CSS</button>
    </p>
...
`;
  ...
  if (_2) {
    ...
    programme.style = {
      scale: 1,
      lineHeight: 40,
      customCSS: `/* 如果設定未生效就加上!important強制最高優先順序 */
header {
...
function clearData() {
  const a = confirm("確定恢復模板字串為預設設定嗎？");
  if (a) {
...
function clearStyle() {
  const a = confirm("確定恢復自定義CSS為預設設定嗎？");
  if (a) {
